  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/5" >
        ...
        <form className="mt-8 flex flex-col gap-4" onSubmit={(e) => handleCreateAd(e)} >
              ...
              >
                {weekDaysArray.map((day, index) => (
                  <ToggleGroup.Item
                    ...
                    title={day.day}
                    className={`w-8 h-8 rounded ${weekDays.includes(index.toString()) ? "bg-violet-500" : "bg-zinc-900"} `}
                  >
                    {day.initial}
                  </ToggleGroup.Item>
                ))}
